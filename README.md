...
check its source, pinact and `prek update` move the pins in the sources, and
vendir fetches the upstream files the ignore and attributes files assemble. This
repository is itself a render of them: `.copier-answers.yml` at the root records
the answers, with `is_template` true, so the root is checked by the same hooks
and workflows as every repository it renders for.
