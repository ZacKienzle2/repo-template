...
def fast(session: nox.Session) -> None:
    ...
    """
    _sync(session)
    session.env["TESTMON_DATAFILE"] = str(Path(session.create_tmp()) / ".testmondata")
    session.run("pytest", "--testmon", "--benchmark-skip", *session.posargs)


@nox.session
def typing(session: nox.Session) -> None:
    """Type-check the tree with the pyright hook, at the version it pins.

    The hook is the one place pyright's version is written; installing pyright
    here as well took whatever release was newest. pyright resolves imports
    through the python on PATH, which is this session's synced environment.
    ...
    _sync(session)
    session.install("pre-commit")
    session.run("pre-commit", "run", "pyright", "--all-files", *session.posargs)
...
def mutants(session: nox.Session) -> None:
    ...
    rendered = tmp / _MUTATION_CONFIG
    rendered.write_text(tomli_w.dumps(config), encoding="utf-8")
    database = str(tmp / _MUTATION_SESSION)

    session.install("cosmic-ray")
    session.run("cosmic-ray", "init", "--force", str(rendered), database)
    session.run("cr-filter-pragma", database)
    session.run("cosmic-ray", "exec", str(rendered), database)
    session.run("cr-report", database)
